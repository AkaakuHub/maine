...
const AnimeCard = ({ anime, priority = false, className }: AnimeCardProps) => {
  ...
  return (
    ...
    >
      <Link href={`/play/${encodeURIComponent(anime.filePath)}`} className="block">
        {/* サムネイル */}
        <div className="relative aspect-video bg-gradient-to-br from-slate-700 to-slate-800 overflow-hidden">
          ...
          {anime.episode && (
            <div className="absolute top-3 left-3">
              <div className="bg-black/70 backdrop-blur-sm px-2 py-1 rounded-md">
                <span className="text-white text-xs font-medium">EP. {anime.episode}</span>
              </div>
            </div>
          )}
          ...
          {anime.year && (
            <div className="absolute top-3 right-3">
              <div className="bg-black/70 backdrop-blur-sm px-2 py-1 rounded-md">
                <span className="text-white text-xs font-medium">{anime.year}</span>
              </div>
            </div>
          )}
        </div>
