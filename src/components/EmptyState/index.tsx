...
const EmptyState = ({ type, searchTerm, onRetry, className }: EmptyStateProps) => {
  const getContent = () => {
    switch (type) {
      case "no-animes":
        return {
      ...
      case "no-search-results":
        return {
          ...
          description: (
            <div className="space-y-2">
              <p>「<span className="font-semibold text-white">{searchTerm}</span>」に一致するアニメはありません。</p>
              <p className="text-sm">
                別のキーワードで検索してみてください。
              </p>
            </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="text-slate-400 mb-8 max-w-md">
        {content.description}
      </div>
