...
import { Play, Calendar, HardDrive, Clock, Info, MoreHorizontal } from "lucide-react";
...
const AnimeListItem = ({ anime }: { anime: AnimeData }) => {
  ...
  return (
    <div className="group bg-slate-800/30 hover:bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 hover:border-purple-400/30 transition-all duration-300">
      <Link href={`/play/${encodeURIComponent(anime.filePath)}`} className="block p-4">
                ...
                <div className="flex items-center gap-4 text-xs text-slate-400">
                  ...
                  {anime.lastWatched && (
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        最後に視聴: {new Date(anime.lastWatched).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                </div>
