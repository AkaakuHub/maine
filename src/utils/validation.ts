...
export function validatePaginationParams(
  ...
  const validatedLimit = Math.max(1, Math.min(100, Number.parseInt(limit || '50', 10) || 50))
...
export function validateSortParams(
  ...
  allowedSortFields: string[] = ['title', 'year', 'episode', 'createdAt', 'lastWatched']
): SortParams {
  const validSortBy = allowedSortFields.includes(sortBy || '') ? sortBy || 'title' : 'title'
